...
# Description: 	This script resamples all input UAV images in a given directory to rasters of a predefined amount of pixels in width and height. 		#
# 				Each resampled raster is then tiled to multiple rasters of 1000 pixels in width and height.												#
# 				The resulting raster files can be used as input to gPb, which is designed to work on images of 1000 x 1000 pixels.						#
# Requirements: All input UAV images should have 3 bands (RGB). If the original data has more bands, they can be cropped in the OSGeo4W Shell with		#
#				gdal_translate -b 1 -b 2 -b 3 input.tif output.tif																						#
#				The input images should then be cropped to a quadratic polygon, which is created with 'Improved Polygon Capturing plugin' with 			#
#				gdalwarp -crop_to_cutline -cutline input.shp input.tif output.tif																		#
#				Watch out that both have the same CRS and the shapefile only contains one attribute														#
...
data_dir = r"D:\..>\directory of UAV data"
...
pix_step = 1000
# Resampling method, available methods in gdalogr:translate {nearest(default),bilinear,cubic,cubicspline,lanczos,average,mode}
res_method = "nearest"
...
for f in files:
	...
	if os.path.splitext(f)[1]=='.tif':
		...
		dir = "Resizing_" + str(os.path.splitext(f)[0]) 
		...
		# (Re)initialize counter for number of tiles
		tiles = 1
		...
		while j <= pix_max:
			...
			processing.runalg('gdalogr:translate',{"INPUT":rlayer, "OUTSIZE":j, "OUTSIZE_PERC":False, "EXPAND":0, "EXTRA": "-r {%s}" % res_method,"PROJWIN":"%f,%f,%f,%f" % (xmin, xmax, ymin, ymax), "OUTPUT":resampled})
			...
			# Increment counter for number of tiles, this works on the assumption that pix_min=1000 and pix_step=1000
			tiles += 1
			...
			for k in range(0,tiles):
				...
				t_xmin = xmin  		
				t_xmax = xmin + (xrange/tiles) 	
				t_ymin = ymin + k*(yrange/tiles)		
				t_ymax = ymin + (k+1)*(yrange/tiles)
				
				# Define name of tiled raster
				tiled = str(os.path.splitext(f)[0] + "_" + str(j) + "pixels_" + res_method +  "_tiling_x0y" + str(k) + ".tif")
				
				# Run tiling of raster and save TIF and TFW File
				processing.runalg('gdalogr:cliprasterbyextent', {"INPUT":reslayer, "PROJWIN":"%f,%f,%f,%f" % (t_xmin,t_xmax,t_ymin,t_ymax), "TFW": True, "OUTPUT":tiled})
				print "--> %s has been tiled and saved as %s in %s\n" % (resampled, tiled, dir)

				# Create all tiles of specific row in x direction
				for l in range (0,tiles-1):
					t_xmin = t_xmax
					t_xmax = t_xmin + (xrange/tiles)
					
					# Define name of tiled raster
					tiled = str(os.path.splitext(f)[0] + "_" + str(j) + "pixels_" +  res_method + "_tiling_x" + str(l+1) + "y" + str(k) + ".tif")
